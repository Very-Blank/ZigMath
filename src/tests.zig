...
test "Vector3 rotate" {
    {
        const vec1: zigmath.vec3.Vector3(f32) = .right;
        const rotated = vec1.rotate(zigmath.quat.Quaternion(f32).createFromRadians(3.141592 / 2.0, zigmath.Constants.Axis.z));
        try std.testing.expectApproxEqAbs(zigmath.vec3.Vector3(f32).up.x, rotated.x, 1e-6);
        try std.testing.expectApproxEqAbs(zigmath.vec3.Vector3(f32).up.y, rotated.y, 1e-6);
        try std.testing.expectApproxEqAbs(zigmath.vec3.Vector3(f32).up.z, rotated.z, 1e-6);
    }
}
